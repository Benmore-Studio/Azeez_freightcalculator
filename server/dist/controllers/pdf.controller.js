...
export async function exportQuotePDF(req, res, next) {
    try {
        ...
        const costBreakdown = quote.costBreakdown || {};
        const tollData = quote.tollData;
        const weatherData = quote.weatherData;
        ...
        const pdfData = {
            ...
            statesCrossed: quote.statesCrossed || [],
            // Rates
...
function buildServiceOptions(quote) {
    const options = [];
    if (quote.isExpedite)
        options.push('Expedite');
    if (quote.isTeam)
        options.push('Team Driver');
    if (quote.isRush)
        options.push('Rush');
    if (quote.isSameDay)
        options.push('Same Day');
    if (quote.isReefer)
        options.push('Refrigerated');
    if (quote.requiresLiftgate)
        options.push('Liftgate');
    if (quote.requiresPalletJack)
        options.push('Pallet Jack');
    if (quote.requiresDriverAssist)
        options.push('Driver Assist');
    if (quote.requiresWhiteGlove)
        options.push('White Glove');
    if (quote.requiresTracking)
        options.push('Tracking');
    return options;
}
