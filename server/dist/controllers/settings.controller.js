...
export async function updateSettings(req, res, next) {
    try {
        ...
        const cleanedData = Object.fromEntries(Object.entries(validatedData).filter(([, value]) => value !== undefined));
 ...
 */
export async function resetDefaults(req, res, next) {
    try {
        if (!req.user) {
            throw ApiError.unauthorized('Authentication required');
        }
 ...
 */
export async function getDefaults(req, res, next) {
    try {
        ...
        const defaults = settingsService.getIndustryDefaults(vehicleType);
