...
export async function lookupByDOT(dotNumber) {
    ...
    try {
        ...
        const riskAssessment = calculateRiskAssessment(carrierInfo, authorities, insurance, safetyRating, safetyData);
        return {
            found: true,
            carrier: carrierInfo,
            authorities,
 ...
 */
export async function verifyCarrier(query) {
 ...
 */
function createNotFoundResult(message) {
    return {
        found: false,
        riskAssessment: {
            level: 'critical',
 ...
 */
function parseEntityType(operation) {
    if (!operation)
        return 'CARRIER';
    const opStr = String(operation).toUpperCase();
    if (opStr.includes('BROKER') && opStr.includes('CARRIER'))
        return 'CARRIER/BROKER';
    if (opStr.includes('BROKER'))
        return 'BROKER';
    return 'CARRIER';
}
/**
 ...
 */
function parseSafetyRating(rating) {
    if (!rating)
        return 'Not Rated';
    const ratingStr = String(rating).toUpperCase();
    if (ratingStr.includes('SATISFACTORY') && !ratingStr.includes('UN'))
        return 'Satisfactory';
    if (ratingStr.includes('CONDITIONAL'))
        return 'Conditional';
    if (ratingStr.includes('UNSATISFACTORY'))
 ...
 */
function calculateRiskAssessment(carrier, authorities, insurance, safetyRating, safetyData) {
    let score = 100;
    const factors = [];
    ...
    // Check authorities
    const hasActiveAuthority = authorities.some(a => a.authorityStatus?.toUpperCase() === 'ACTIVE' ||
...
//# sourceMappingURL=fmcsa.service.js.map