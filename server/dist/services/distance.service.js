 ...
 */
export async function calculateDistance(origin, destination, vehicleSpecs) {
 ...
 */
async function calculateDistancePCMiler(origin, destination, vehicleSpecs) {
    ...
    try {
        ...
        const totalMilesLine = mileageReport.find((line) => line.LMiles || line.TMiles || line.Miles);
 ...
 */
function mapHazmatTypeNumeric(type) {
    const mapping = {
        none: 0,
        general: 1,
        explosive: 2,
 ...
 */
export async function getRouteDetails(origin, destination, vehicleSpecs) {
    // Try PC*MILER first
 ...
 */
async function geocodeWithPCMiler(location, apiKey) {
    try {
...
async function getRouteDetailsPCMiler(origin, destination, vehicleSpecs) {
    const apiKey = env.apiKeys.pcmiler;
    if (!apiKey) {
    ...
    try {
        ...
        console.log('[PC*MILER] Report types received:', data.map((r) => r.__type || 'unknown'));
        // Parse mileage report - last stop has totals
        const mileageReport = data.find((r) => r.__type?.includes('MileageReport'));
 ...
 */
async function getRouteDetailsGoogle(origin, destination) {
    const apiKey = env.apiKeys.googleMaps;
    if (!apiKey) {
 ...
 */
export function estimateMilesFallback(originLat, originLng, destLat, destLng) {
 ...
 */
export function getVehicleDefaults(vehicleType) {
