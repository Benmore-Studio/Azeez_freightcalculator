 ...
 */
export async function getWeatherForecast(lat, lng, targetDate) {
 ...
 */
function parseCurrentWeather(data, date) {
 ...
 */
function parseForecast(data, targetDate) {
 ...
 */
export async function getRouteWeather(originLat, originLng, destLat, destLng, deliveryDate) {
    ...
    const routeCondition = getWorstCondition(originWeather?.condition || 'normal', destWeather?.condition || 'normal');
 ...
 */
function getWorstCondition(condition1, condition2) {
    const severity = {
        normal: 0,
        fog: 1,
        light_rain: 2,
 ...
 */
function calculateRiskLevel(condition, origin, dest) {
    ...
    switch (condition) {
        ...
        case 'light_rain':
            // Check visibility
            const minVisibility = Math.min(origin?.visibility || 10, dest?.visibility || 10);
            if (minVisibility < 1)
                return 'high';
            if (minVisibility < 3)
                return 'moderate';
 ...
 */
function generateAdvisories(condition, origin, dest) {
    const advisories = [];
    switch (condition) {
        case 'extreme_weather':
    ...
    const minTemp = Math.min(origin?.temperature || 70, dest?.temperature || 70);
    const maxTemp = Math.max(origin?.temperature || 70, dest?.temperature || 70);
    if (minTemp < 32) {
    ...
    // Wind advisories
    const maxWind = Math.max(origin?.windSpeed || 0, dest?.windSpeed || 0);
    if (maxWind > 40) {
