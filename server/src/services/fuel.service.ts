...
const PADD_SERIES: Record<string, string> = {
  // PADD 1 - East Coast
 ...
 */
export async function getFuelPriceForState(stateCode: string): Promise<FuelPriceResult> {
 ...
 */
export async function getRouteFuelPrice(statesCrossed: string[]): Promise<FuelPriceResult> {
  ...
  const prices = await Promise.all(
    statesCrossed.map(state => getFuelPriceForState(state))
  );
 ...
 */
async function fetchFuelPriceFromEIA(region: string): Promise<number | null> {
 ...
 */
async function getCachedFuelPrice(stateCode: string): Promise<{
  pricePerGallon: number;
  region: string;
  fetchedAt: Date;
} | null> {
 ...
 */
async function cacheFuelPrice(
  stateCode: string,
  price: number,
  _region: string // Not stored in DB, just for logging
): Promise<void> {
  try {
 ...
 */
export async function updateAllFuelPrices(): Promise<{
  updated: number;
  failed: number;
}> {
  const apiKey = env.apiKeys.eia;
  ...
  for (const [region, seriesId] of Object.entries(PADD_SERIES)) {
    try {
      ...
      if (price) {
        ...
        const representativeStates: Record<string, string> = {
          PADD1: 'NY',
          PADD1A: 'MA',
          PADD1B: 'PA',
          PADD1C: 'FL',
 ...
 */
export async function getNationalAverageFuelPrice(): Promise<FuelPriceResult> {
  return getFuelPriceForState('US');
}
